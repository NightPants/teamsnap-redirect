...
def populate_team_picker():
    global team_picker, team_options_list
    if team_picker:
        team_picker.destroy()
    ...
    team_picker.event_generate("<<ComboboxSelected>>")

def on_team_selected(event):
    global team_id_var, team_division_var
    selected_value = team_picker.get()
    if selected_value == "All Teams":
        team_id_var.set("all")
        team_division_var.set("")
    else:
        for name, division, team_id in team_options_list:
            if f"{name} ({division})" == selected_value:
                team_id_var.set(team_id)
                team_division_var.set(division)
                team_name_var.set(name) # Update team_name_var with the selected name
                return
