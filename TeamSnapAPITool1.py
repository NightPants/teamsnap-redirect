...
def refresh_teams_now():
    global access_token, teams, refresh_now_button, team_info_frame, last_refreshed_label, progress_bar_refresh_teams
    if not access_token:
...
def fetch_events_threaded():
    ...
    try:
        ...
        root.after(0, populate_results_table, events, event_type_str) # Pass event_type_str
...
def populate_results_table(events, event_type_str):
    ...
    if events:
        ...
        messagebox.showinfo("Info", f"No {event_type_str.lower()} found for this date.") # Now event_type_str is defined
    ...
    if filename:
        try:
            with open(filename, 'w', newline='') as csvfile:
                fieldnames = ['Time', 'Location', 'Team Name', 'Opponent', 'Division', 'Event Type', 'Game Type Code', 'Is Canceled', 'League Controlled']
                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                writer.writeheader()
